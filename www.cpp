...
int main(int argc, char *argv[]){
  ...
  if(argv[1] == NULL)
  {
  cout << "Please specify a port number." ; //check if the user has input a port number
  return 0;
  }
...
void *serve(void *s) {
 ...
 write(sock, "Server: El Mundio/1.3.3.7\r\n", strlen("Content-Type: text/html\r\n\r\n"));
 ...
 goto logfile; //skip to logfile creation
  ...
  if(access(fullpath,F_OK) < 0 ) { //if the file doesn't exist, print a 404 error with hardcoded html, else 

	strcat(rep, " 404 File not found\r\n");
	strcat(rep, "Connection: close\r\n");
	strcat(rep, "Server: El Mundio/1.3.3.7\r\n");
	strcat(rep, "Content-Type: text/html\r\n\r\n");
	...
	write(sock,rep,strlen(rep)); //write the reply to the socket
	goto logfile; //skip to logfile creation;
  	} 
	else 
	{
...
write(sock, "Server: El Mundio/1.3.3.7", strlen("Server: El Mundio/1.3.3.7")); //send server name
write(sock, "\r\n", strlen("\r\n")); //send new line

if((strcmp(exts,".cgi")==0)) //if a cgi script
{

int pid = fork(); //create a new thread
...
if((strcmp(exts,".jpg")==0) ||  (strcmp(exts,".jpeg")==0) || (strcmp(exts,".png")==0))
{
...
{
write(sock,line, 2048); //read the file 2048 bits at a time, and write to the socket
readf = read(fd,line,2048);
}
...
unsigned short port;
  ...
  r=getpeername(s,(struct sockaddr *) &cliaddr,&cliaddrlen);
    ...
    hostip = inet_ntoa(cliaddr.sin_addr);  //get the IP address of the client
    //port = ntohs(cliaddr.sin_port); //get the port number
...
{
ofstream log ("log.txt", ios::out | ios::app); //for some reason, creat() makes it read only
}

fd = open("log.txt",O_WRONLY|O_APPEND); //open the log file
